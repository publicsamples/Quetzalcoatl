...
namespace Mod2_t_parameters
{
// Parameter list for Mod2_impl::Mod2_t ------------------------------------------------------------
...
template <int NV>
using multi_0 = parameter::from0To1<Mod2_impl::tempo_sync_t<NV>, 
                                    1, 
                                    multi_0Range>;
...
using packs_0 = parameter::from0To1<Mod2_impl::pack_resizer_t, 
                                    0, 
                                    packs_0Range>;
...
using onshot = parameter::chain<ranges::Identity, 
                                parameter::plain<Mod2_impl::ramp_t<NV>, 1>, 
                                parameter::plain<Mod2_impl::input_toggle_t<NV>, 0>>;

template <int NV>
using tempo = parameter::plain<Mod2_impl::tempo_sync_t<NV>, 
                               0>;
template <int NV>
using type = parameter::plain<Mod2_impl::branch_t<NV>, 
                              0>;
template <int NV>
using a = parameter::plain<Mod2_impl::ahdsr_t<NV>, 0>;
template <int NV>
using d = parameter::plain<Mod2_impl::ahdsr_t<NV>, 3>;
template <int NV>
using s = parameter::plain<Mod2_impl::ahdsr_t<NV>, 4>;
template <int NV>
using r = parameter::plain<Mod2_impl::ahdsr_t<NV>, 5>;
template <int NV>
using h = parameter::plain<Mod2_impl::ahdsr_t<NV>, 2>;
template <int NV>
using crv = parameter::plain<Mod2_impl::ahdsr_t<NV>, 6>;
template <int NV>
using Mod2_t_plist = parameter::list<tempo<NV>, 
                                     multi<NV>, 
                                     packs, 
                                     type<NV>, 
                                     onshot<NV>, 
...
template <int NV>
using Mod2_t_ = container::chain<Mod2_t_parameters::Mod2_t_plist<NV>, 
                                 wrap::fix<2, modchain_t<NV>>>;

// =================================| Root node initialiser class |=================================

template <int NV> struct instance: public Mod2_impl::Mod2_t_<NV>
{
	...
	struct metadata
	{
		...
		SNEX_METADATA_ENCODED_PARAMETERS(160)
		{
            ...
            0x4200, 0x0000, 0x4060, 0x0000, 0x3F80, 0xCCCD, 0x3DCC, 0x025B, 
            0x0000, 0x7000, 0x6361, 0x736B, 0x0000, 0x8000, 0x0024, 0x8000, 
            0x9841, 0x8B1E, 0x0040, 0x8000, 0x003F, 0x0000, 0x5B00, 0x0003, 
            0x0000, 0x7974, 0x6570, 0x0000, 0x0000, 0x0000, 0x0000, 0xCD40, 
            0x8CCC, 0x003F, 0x8000, 0xCD3F, 0xCCCC, 0x5B3D, 0x0004, 0x0000, 
            0x6E6F, 0x6873, 0x746F, 0x0000, 0x0000, 0x0000, 0x8000, 0x003F, 
            0x0000, 0x0000, 0x8000, 0x003F, 0x8000, 0x5B3F, 0x0005, 0x0000, 
	...
	instance()
	{
		...
		auto& modchain = this->getT(0);                             // Mod2_impl::modchain_t<NV>
		auto& chain = this->getT(0).getT(0);                        // Mod2_impl::chain_t<NV>
		auto& tempo_sync = this->getT(0).getT(0).getT(0);           // Mod2_impl::tempo_sync_t<NV>
		auto& ramp = this->getT(0).getT(0).getT(1);                 // Mod2_impl::ramp_t<NV>
		auto& clear = this->getT(0).getT(0).getT(2);                // math::clear<NV>
		auto& branch = this->getT(0).getT(1);                       // Mod2_impl::branch_t<NV>
		auto& chain3 = this->getT(0).getT(1).getT(0);               // Mod2_impl::chain3_t<NV>
		auto& pack_resizer = this->getT(0).getT(1).getT(0).getT(0); // Mod2_impl::pack_resizer_t
		auto& cable_pack = this->getT(0).getT(1).getT(0).getT(1);   // Mod2_impl::cable_pack_t<NV>
		auto& add1 = this->getT(0).getT(1).getT(0).getT(2);         // math::add<NV>
		auto& chain4 = this->getT(0).getT(1).getT(1);               // Mod2_impl::chain4_t<NV>
		auto& cable_table = this->getT(0).getT(1).getT(1).getT(0);  // Mod2_impl::cable_table_t<NV>
		auto& add2 = this->getT(0).getT(1).getT(1).getT(1);         // math::add<NV>
		auto& chain5 = this->getT(0).getT(1).getT(2);               // Mod2_impl::chain5_t<NV>
		auto& add4 = this->getT(0).getT(1).getT(2).getT(0);         // math::add<NV>
		auto& rect = this->getT(0).getT(1).getT(2).getT(1);         // math::rect<NV>
		auto& peak1 = this->getT(0).getT(1).getT(2).getT(2);        // Mod2_impl::peak1_t<NV>
		auto& clear1 = this->getT(0).getT(1).getT(2).getT(3);       // math::clear<NV>
		auto& input_toggle = this->getT(0).getT(1).getT(2).getT(4); // Mod2_impl::input_toggle_t<NV>
		auto& ahdsr = this->getT(0).getT(1).getT(2).getT(5);        // Mod2_impl::ahdsr_t<NV>
		auto& add3 = this->getT(0).getT(1).getT(2).getT(6);         // math::add<NV>
		auto& peak = this->getT(0).getT(2);                         // Mod2_impl::peak_t<NV>
		auto& event_data_writer = this->getT(0).getT(3);            // routing::event_data_writer<NV>
		auto& chain2 = this->getT(0).getT(4);                       // Mod2_impl::chain2_t<NV>
		auto& ahdsr1 = this->getT(0).getT(4).getT(0);               // Mod2_impl::ahdsr1_t<NV>
		auto& voice_manager2 = this->getT(0).getT(4).getT(1);       // envelope::voice_manager
		...
		event_data_writer.setParameterT(0, 1.); // routing::event_data_writer::SlotIndex
		...
		this->setParameterT(1, 3.5);
		this->setParameterT(2, 4.34748);
		this->setParameterT(3, 1.1);
	...
	void setExternalData(const ExternalData& b, int index)
	{
		...
		this->getT(0).getT(0).getT(1).setExternalData(b, index);         // Mod2_impl::ramp_t<NV>
		this->getT(0).getT(1).getT(0).getT(0).setExternalData(b, index); // Mod2_impl::pack_resizer_t
		this->getT(0).getT(1).getT(0).getT(1).setExternalData(b, index); // Mod2_impl::cable_pack_t<NV>
		this->getT(0).getT(1).getT(1).getT(0).setExternalData(b, index); // Mod2_impl::cable_table_t<NV>
		this->getT(0).getT(1).getT(2).getT(2).setExternalData(b, index); // Mod2_impl::peak1_t<NV>
		this->getT(0).getT(1).getT(2).getT(5).setExternalData(b, index); // Mod2_impl::ahdsr_t<NV>
		this->getT(0).getT(2).setExternalData(b, index);                 // Mod2_impl::peak_t<NV>
		this->getT(0).getT(4).getT(0).setExternalData(b, index);         // Mod2_impl::ahdsr1_t<NV>
	}
};
}
...
template <int NV>
using Mod2 = wrap::node<Mod2_impl::instance<NV>>;
}
