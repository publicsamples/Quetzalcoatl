...
template <int NV>
using multi_0 = parameter::from0To1<Mod1_impl::tempo_sync_t<NV>, 
                                    1, 
                                    multi_0Range>;
...
using packs_0 = parameter::from0To1<Mod1_impl::pack_resizer_t, 
                                    0, 
                                    packs_0Range>;
...
using onshot = parameter::chain<ranges::Identity, 
                                parameter::plain<Mod1_impl::ramp_t<NV>, 1>, 
                                parameter::plain<Mod1_impl::input_toggle_t<NV>, 0>>;

template <int NV>
using tempo = parameter::plain<Mod1_impl::tempo_sync_t<NV>, 
                               0>;
template <int NV>
using type = parameter::plain<Mod1_impl::branch_t<NV>, 
                              0>;
template <int NV>
using a = parameter::plain<Mod1_impl::ahdsr_t<NV>, 0>;
template <int NV>
using d = parameter::plain<Mod1_impl::ahdsr_t<NV>, 3>;
template <int NV>
using s = parameter::plain<Mod1_impl::ahdsr_t<NV>, 4>;
template <int NV>
using r = parameter::plain<Mod1_impl::ahdsr_t<NV>, 5>;
template <int NV>
using h = parameter::plain<Mod1_impl::ahdsr_t<NV>, 2>;
template <int NV>
...
template <int NV> struct instance: public Mod1_impl::Mod1_t_<NV>
{
	...
	struct metadata
	{
		...
		SNEX_METADATA_ENCODED_PARAMETERS(160)
		{
            ...
            0x4200, 0x3333, 0x4083, 0x0000, 0x3F80, 0xCCCD, 0x3DCC, 0x025B, 
	...
	instance()
	{
		...
		auto& modchain = this->getT(0);                             // Mod1_impl::modchain_t<NV>
		auto& chain = this->getT(0).getT(0);                        // Mod1_impl::chain_t<NV>
		auto& tempo_sync = this->getT(0).getT(0).getT(0);           // Mod1_impl::tempo_sync_t<NV>
		auto& ramp = this->getT(0).getT(0).getT(1);                 // Mod1_impl::ramp_t<NV>
		auto& clear = this->getT(0).getT(0).getT(2);                // math::clear<NV>
		auto& branch = this->getT(0).getT(1);                       // Mod1_impl::branch_t<NV>
		auto& chain3 = this->getT(0).getT(1).getT(0);               // Mod1_impl::chain3_t<NV>
		auto& pack_resizer = this->getT(0).getT(1).getT(0).getT(0); // Mod1_impl::pack_resizer_t
		auto& cable_pack = this->getT(0).getT(1).getT(0).getT(1);   // Mod1_impl::cable_pack_t<NV>
		auto& add1 = this->getT(0).getT(1).getT(0).getT(2);         // math::add<NV>
		auto& chain4 = this->getT(0).getT(1).getT(1);               // Mod1_impl::chain4_t<NV>
		auto& cable_table = this->getT(0).getT(1).getT(1).getT(0);  // Mod1_impl::cable_table_t<NV>
		auto& add2 = this->getT(0).getT(1).getT(1).getT(1);         // math::add<NV>
		auto& chain5 = this->getT(0).getT(1).getT(2);               // Mod1_impl::chain5_t<NV>
		auto& add4 = this->getT(0).getT(1).getT(2).getT(0);         // math::add<NV>
		auto& rect = this->getT(0).getT(1).getT(2).getT(1);         // math::rect<NV>
		auto& peak1 = this->getT(0).getT(1).getT(2).getT(2);        // Mod1_impl::peak1_t<NV>
	...
	void setExternalData(const ExternalData& b, int index)
	{
		...
		this->getT(0).getT(0).getT(1).setExternalData(b, index);         // Mod1_impl::ramp_t<NV>
		this->getT(0).getT(1).getT(0).getT(0).setExternalData(b, index); // Mod1_impl::pack_resizer_t
		this->getT(0).getT(1).getT(0).getT(1).setExternalData(b, index); // Mod1_impl::cable_pack_t<NV>
